...
/* ── Section title — cabecera de sección limpia, NO parece botón ───── */
/*
   Patrón correcto para Streamlit:
   st.markdown('<p class="ios-section-title">Título</p>', ...)
   st.pyplot(...)   ← Streamlit lo renderiza fuera del HTML, esto está bien

   El fondo/sombra se da al bloque Streamlit nativo, no a un div wrapper.
*/
.ios-section-title {
...
/* ── Content panel — superficie Liquid Glass para agrupar contenido ── */
/*
   Se usa como wrapper SOLO para contenido HTML puro (alerts, chain blocks,
   listas de recomendaciones). NUNCA para envolver st.pyplot / st.dataframe.
*/
.ios-panel {
...
/* Compatibilidad — .ios-card-title por si queda alguna referencia inline */
.ios-card-title {
    font-size: 11px;
    font-weight: 700;
    text-transform: uppercase;
...
/* ── Dar superficie glass a los bloques nativos de Streamlit ────────── */
/* Los gráficos pyplot y dataframes de Streamlit renderizan dentro de   */
/* div[data-testid="stVerticalBlock"] > div. Usamos el column wrapper.  */

/* Columnas con gráficos — fondo suave sin hover de "botón" */
...
if pagina == "📊  Panel General":
    ...
    with col_a:
        st.markdown('<p class="ios-section-title">Distribución de Notas</p>', unsafe_allow_html=True)
        st.pyplot(fig_distribucion_notas(df), use_container_width=True)

    with col_b:
        st.markdown('<p class="ios-section-title">Asistencia vs Rendimiento</p>', unsafe_allow_html=True)
        st.pyplot(fig_asistencia_vs_nota(df), use_container_width=True)

    st.markdown("<br>", unsafe_allow_html=True)

    # Tabla de estudiantes
    st.markdown('<p class="ios-section-title">Listado de Estudiantes</p>', unsafe_allow_html=True)
...
# ===========================================================================
    ...
    # Recomendaciones
    st.markdown('<p class="ios-section-title">Recomendaciones Personalizadas</p>', unsafe_allow_html=True)
    recs = recomendaciones(row)
    st.markdown(
        '<div class="ios-panel">' +
        "".join(f'<div style="padding:4px 0;font-size:14px;color:var(--label-primary);">{r}</div>' for r in recs) +
        '</div>',
        unsafe_allow_html=True
    )
...
# ===========================================================================
    ...
    # --- AES
    with tab1:
        st.markdown('<p class="ios-section-title">Encriptación AES-256-GCM</p>', unsafe_allow_html=True)
        st.markdown("""
        ...
        # Demo integridad
        st.markdown('<p class="ios-section-title">Demo — Detección de Manipulación</p>', unsafe_allow_html=True)
        st.markdown("""
    ...
    # --- RSA
    with tab2:
        st.markdown('<p class="ios-section-title">Firma Digital RSA-2048</p>', unsafe_allow_html=True)
        st.markdown("""
        <div class="ios-alert ios-alert-info">
    ...
    with tab3:
        st.markdown('<p class="ios-section-title">Cifrado Masivo de Datos Sensibles</p>', unsafe_allow_html=True)
        st.markdown("""
        <div class="ios-alert ios-alert-info">
...
# ===========================================================================
    ...
    with col1:
        st.markdown('<p class="ios-section-title">Registrar Evento Educativo</p>', unsafe_allow_html=True)
    ...
    with col2:
        st.markdown('<p class="ios-section-title">Verificación de Integridad</p>', unsafe_allow_html=True)
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown('<p class="ios-section-title">Visualización de la Cadena</p>', unsafe_allow_html=True)

    chain_data = blockchain.get_resumen_cadena()
    chain_html = ""
    for i, bloque_res in enumerate(chain_data[-8:]):
        is_genesis = bloque_res["Bloque"] == 0
        bg = "rgba(0,122,255,0.06)" if is_genesis else "var(--glass-bg-heavy)"
        border = "rgba(0,122,255,0.35)" if is_genesis else "var(--glass-border)"
        arrow = '<div class="chain-arrow">↕</div>' if i < len(chain_data[-8:]) - 1 else ''
        firma_html = f'<div class="chain-hash">✍️&nbsp; firma: {bloque_res["Firma"]}</div>' if bloque_res["Firma"] != "—" else ""
        chain_html += f"""
...
# ===========================================================================
    ...
    st.markdown('<p class="ios-section-title">Modelo de Predicción de Riesgo — Random Forest</p>', unsafe_allow_html=True)
    st.markdown("""
    <div class="ios-alert ios-alert-info">
